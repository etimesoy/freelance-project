...
<b>Пожалуйста, напишите название вашего проекта</b>"""
    ...
    await state.set_state("feedback__get_project_name")


@dp.message_handler(state="feedback__get_project_name")
async def get_project_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(message_text, reply_markup=stars_keyboard)
    await state.set_state("feedback__get_assessment_of_work")


@dp.callback_query_handler(stars_callback.filter(), state="feedback__get_assessment_of_work")
...
async def send_gratitude_response(message: types.Message, state: FSMContext):
    ...
    if stars_count in ["1", "2", "3"]:
        ...
        await state.set_state("feedback__get_gratitude")
...
async def send_suggestions_for_improvements_message(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state("feedback__get_suggestions_for_improvements_message")


@rate_limit(limit=180)
@dp.message_handler(state="feedback__get_suggestions_for_improvements_message")
